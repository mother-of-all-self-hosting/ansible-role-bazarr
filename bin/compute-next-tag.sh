...
#
# Tags look like `v<bazarr version>-<release>`, which is what this repository
# has always published (v1.5.6-ls342-1 ... v1.6.0-1):
#
# - if defaults/main.yml points at a Bazarr version that has never been
#   released, the release counter restarts at 0 (`v1.6.1-0`)
# - otherwise the counter is incremented (`v1.6.1-1`), but only if something
...
# The commit-message approach this replaced only ever produced `-0` tags, and
# only for commits authored by renovate[bot] within the last 20 commits, so
# every other change to the role went unreleased until somebody tagged by hand.
...
# Anchored on `bazarr_version:` so that neither the Renovate annotation above it
# nor `bazarr_container_image_tag`, which is derived from it, can be mistaken
# for it.
version="$(sed -nE 's|^bazarr_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Bazarr version from $defaults_path"
	exit 1
fi

# Bazarr's own version is carried without a leading `v` (the tags this
# repository publishes do have one), but tolerate one so that a future change of
